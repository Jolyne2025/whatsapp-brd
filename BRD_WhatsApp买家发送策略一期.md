# WhatsApp Buyer Messaging Strategy (Phase 1) BRD ｜ WhatsApp 买家发送策略（一期）BRD

| Item 项 | Content 内容 |
|----|------|
| Document Status 文档状态 | v1.2 Release (shared with collaboration teams) · v1.2 发布版（协作团队共享） |
| Date 日期 | 2026-08-17 |
| Author 作者 | Jolyne Zhang（Lifecycle / Buyer Side 买家侧） |
| Reviewer 评审人 | Shuhe 述合 |
| Related Documents 关联文档 | Visable WhatsApp Project Proposal v0.1 draft; 04 WhatsApp business strategy table; WhatsApp message access feasibility assessment (Confluence ARISE/436797534); Alibaba International WhatsApp data & strategy summary |

---

## 1. Project Overview（项目概述）

### 1.1 Background & Opportunity（背景与机会）

| Fact 事实 | Basis 依据 | Implication 推论 |
|------|------|------|
...
### 1.4 Phase 1 Goals（一期目标，按消息类型拆分，日均漏斗口径）

**North Star (total 合计)：Daily AB2 ≥ 8**（by FY26/27 year-end 2027-03-31）＝ **System messages contribution (floor ≥3)** ＋ **Marketing messages contribution (floor ≥5)**. Contribution split locked via AB2 conversations analysis (under calculation).

**A. System message goals（系统消息目标，细节负责：Utkarsha · PRD_Reminder 口径）——activate existing stock, event-driven, no dependence on subscription pool ramp-up:**

| Objective 目标项 | Target 数值 |
|--------|------|
| Follow-up coverage (unanswered requests 未回复 request) | 100% (once per request; skip if already replied; stop after AB2 established) |
...
**B. Marketing message goals（营销消息目标，负责：Jolyne）——incremental acquisition, subscription-pool funnel:**
...
**C. Global guardrails (shared by both types 全局护栏)：** delivery rate ≥ 90%, 48h read rate ≥ 50%, unsubscribe rate < 5%, complaint/block rate < 2%.

**中文对照：** 送达率 ≥ 90%、48h 已读率 ≥ 50%、退订率 < 5%、投诉/拉黑率 < 2%。

---

## 2. Benchmark Baseline（对标基线，日均口径）

### 2.1 Marketing Funnel Baseline（营销漏斗基线，业务策略表 V 目标基线，日均口径，订阅池驱动）

**Funnel breakdown 漏斗拆分说明**: North Star daily AB2 ≥ 8 is delivered by **two independent tracks**——① **system messages** (event-driven: follow-up + conversation reminders, activating existing conversations, target contribution ≥ 3, see 4.1 / PRD_Reminder) ② **marketing messages** (subscription-pool funnel, target contribution ≥ 8.8, i.e. table below). Marketing frequency benchmarked to Alibaba: FY26 target **2 rounds/week/user** (Yuque "WhatsApp Annual Review & Strategy Planning"); EU compliance stricter, Visable starts at **1 round/week**, A/B validation cap of 2 rounds/week.

| Funnel stage 漏斗层级 | Derivation 推导 | Alibaba (benchmark 对标) | V target (FY26/27 year-end) |
|---------|------|---------------|-------------------|
...
> **Frequency basis 频次口径**: Alibaba FY26 target is **2 rounds/week/user** (Yuque "WhatsApp Annual Review & Strategy Planning": weekly reach 1.7M ÷ subscription pool); 328K daily UV achieved by 2026-03. Visable starts at **1 round/week** (14%/day, full pool in 7 days), CTR conservatively 10% × 0.6 decay (frequency doubling → open rate decline, Alibaba FY26 basis); A/B validates **2 rounds/week** (28%/day) during pilot.
...
## 3. Phase 1 Scope（一期范围界定）

### 3.1 In Scope（一期核心）

1. **Subscription collection 订阅收集链路**: standalone checkbox on registration page (unchecked by default) + second ask after phone verification + popup after inquiry submission + Subscription Center unified management (reuse UPR-55 Lifecycle entry)
2. **System/transactional messages launch 系统/交易类消息首发** (details: Utkarsha · PRD_Reminder): two buyer-side system messages (① conversation reminder: notify buyer when supplier replies; ② buyer follow-up reminder: nudge if no reply by day after supplier reply, directly serves AB2 North Star, reuse PRD_Reminder rules), all via Utility templates. Billing: free within WhatsApp 24h customer-service window (recent message in buyer's dialog); paid Utility template outside window (~€0.03–0.04/message; on-site behavior does not open a window)
3. **Marketing pilot 营销试点** (BRD focus, owner: Jolyne, see 4.2): two-strategy second touch (RFQ marketing + product recommendation)
4. **Sending infrastructure 发送基础设施**: three-state subscription service (subscribed/unsubscribed/unknown), frequency capping (24h/once per relationship pair, ≤3/day per user), tracking loop (push_id + visitor_sk attribution)
5. **Templates 模板**: Utility first batch EN+DE, neutral wording; Marketing templates enter review process but not sent at scale yet
...
### 4.1 Send Scenarios · System/Transactional（发送场景 · 系统/交易类，一期优先，合规压力小）
...
| Scenario 场景 | Trigger 触发时机 | Message content 消息内容 | Compliance type 合规类型 | Relation to North Star AB2 与北极星 AB2 的关系 |
|------|---------|---------|---------|-------------------|
| ① Conversation reminder (supplier reply → notify buyer) | Real-time event after supplier reply | Quote/reply card + AI-summarized key fields (price range/MOQ/lead time) + one-tap reply | Utility (transactional 交易通知) | Indirect: keeps conversation warm, accelerates first response (≤5min → AB2 rate 59.6%) |
| ② Buyer follow-up reminder (buyer no reply → second nudge) | Day after supplier reply, buyer still hasn't replied (PRD basis: wlw 10am / ep 9am) | Follow-up copy (reuse PRD templates: "Supplier Waiting for Your Reply" etc.) | Utility (transactional 交易通知) | **Direct: buyer response to positive connection = AB2** |
...
**Follow-up guardrails 催复护栏（承接 PRD_Reminder 既有规则）:** one follow-up per request/message; skip if already replied; open status only (no send for expired / frozen / not interested / archived); stop after AB2 established.

**中文对照：** 每个 request/message 仅发一次催复；用户已回复不发；仅 open 状态（expired / frozen / not interested / archived 不发）；AB2 建立后不再发。

> **Billing path 计费路径**: free if both messages land inside the WhatsApp 24h customer-service window (recent message in buyer's dialog); paid Utility template outside the window (~€0.03–0.04/message; on-site behavior does not open a window). Follow-up timing consistent with PRD (next-day 10am/9am), different time slots from marketing second touch (4.2) to avoid repeated disturbance of the same user.
...
### 4.2 Send Scenarios · Marketing（发送场景 · 营销类，负责：Jolyne；一期试点核心：两大策略二次触达）
...
| Strategy 策略 | PUSH counterpart | EDM counterpart | Current volume 当前量级 | Open rate 打开率（wlw / ep） | Rationale 选它的理由 |
|------|----------|---------|---------|-------------------|-----------|
...
**Assets & landing pages 素材与落地页（复用既有 Push 资产，不新造）:**
...
**Why these two 为什么是它（试点选型依据）:**

| Basis 依据 | Explanation 说明 |
|------|------|
...
**Audience pools 圈人池（按触达轮次滚动）:**

| Pool 池 | Selection condition 圈人条件 | Size 规模 |
|----|---------|--------|
...
**中文对照：** 两个池互斥去重；发送前过订阅态检查（必须"已订阅"），实际发送量以已订阅子集为准。

**Send flow 发送流程（与现有渠道衔接，每周 1 轮滚动）:**
...
**Pilot measurement 试点衡量（跑通验证两个命题）:**

| Proposition 命题 | Validation method 验证方式 | Pass criteria 达标口径 |
|------|---------|---------|
...
**Pilot cadence 试点节奏:** DE single-country canary (after Legal sign-off + Marketing template approval) → run 2–3 rounds (**1 round/week**) to evaluate → frequency A/B (1 vs 2 rounds/week; observe disturbance/unsubscribe/complaints) → expand countries/scale after passing (FR/IT/ES following template review).

**中文对照：** DE 单国灰度（Legal sign-off + Marketing 模板过审后启动）→ 跑通 2-3 轮（**每周 1 轮**）评估 → 频次 A/B（每周 1 次 vs 每周 2 次，观察打扰率/退订/投诉）→ 达标后扩国/扩容（FR/IT/ES 随模板送审推进）。

### 4.3 Subscription Scenarios（订阅场景，收集触点 + double opt-in 方案）

| Touchpoint 触点 | Location 位置 | Level-1 consent 一级同意机制 | Level-2 confirmation 二级确认机制 |
|------|------|------------|-------------|
...
**Marketing double opt-in solution 营销 double opt-in 解决方案（一期必须落地）:**
...
### 4.4 Send Strategy（发送策略，与现有渠道的协同关系）

- **Cadence 节奏**: WhatsApp ranks after Visable's existing PUSH/EDM touches; **second touch** on users who responded to existing touches; frequency **starts at 1 round/week** (benchmarked to Alibaba FY26 marketing target of 2 rounds/week; EU compliance starts lower; pilot A/B caps at 2 rounds/week)——a "carry + amplify" relationship with existing PUSH/EDM, not a parallel independent channel.
- **Trigger timing 触发时机**: marketing second touch at 10:00 AM buyer local time (Alibaba BRD §5.4 experience); system: conversation reminder real-time on event, follow-up reminder per PRD_Reminder cadence (next day after supplier reply, wlw 10am / ep 9am).
- **Languages 多语种**: DE/EN start; FR/IT/ES follow template review.
- **Fatigue 疲劳度**: EU tolerance is low, capping strict (24h/1 per relationship pair, ≤3 per user per 24h, global daily circuit breaker ≤ 15K sessions against script attacks).
...
### 4.5 Explicitly Out of Scope（明确不做的红线）

| Red line 红线 | Explanation 说明 |
|------|------|
...
## 5. Technical Implementation & Dependencies（技术实现与依赖）
...
### 5.2 Dependency List（依赖清单）

| Dependency 依赖 | Owner 负责方 | Key date 关键日期 |
|------|--------|---------|
| Subscription entry changes (registration checkbox / post-verification ask / inquiry popup) | Product (Muke 牧可) | Schedule locked within August |
...
| Meta Cloud API channel & template submission | Harry / Shuhe 述合 | September (Utility first batch) |
...
## 6. Risks & Compliance（风险与合规）

| Risk 风险 | Severity 严重度 | Mitigation 缓解 |
|------|--------|------|
...
Compliance prerequisites 合规前置: GDPR Art.7 + ePrivacy explicit consent, double opt-in, evidence retention (who/when/entry/exact wording), immediate unsubscribe, Abmahnung risk.

**中文对照：** GDPR Art.7 + ePrivacy 明示同意、double opt-in、留证（谁/何时/入口/文案原文）、退订即时生效、Abmahnung 风险。

---

## 7. Monthly Cost Estimate（月度费用测算）

**Billing model 计费模型（Meta WhatsApp 按量计费）**: no account application fee / fixed monthly fee / subscription fee; billed per **template message** only (price varies by category); conversation messages within the 24h customer-service window are free.

| Message category 消息类别 | Unit price 单价 | Scope 适用范围 |
|---------|------|---------|
| Marketing 营销类 | ~€0.11/message | Marketing second touch (two strategies) |
| Utility 交易类 | ~€0.03–0.04/message (use €0.035) | System messages (conversation reminder / follow-up reminder) |
...
**Monthly cost estimate 月度费用估算（随订阅池爬坡递增，非稳态）:**
...
| Month 月份 | Phase 阶段 | Mid-month cumulative subscriptions 月中累计订阅（假设） | Reachable 可触达（60%） | Daily sends 日均发送（每周 1 轮，14%/日） | Marketing monthly cost 营销月费用 | System monthly cost 系统消息月费用 | Total 合计 |
|------|------|-------------------|-------------|----------------------------|-----------|-------------|------|
...
| Time 时间 | Milestone 里程碑 | Deliverable 交付 |
|------|--------|------|
...
### 9.1 Funnel Basis & Key Assumptions（漏斗口径与关键假设，推导细节）

**Funnel basis & key assumptions 漏斗口径与关键假设**: all funnel metrics from send onwards are **daily averages**. Key assumptions: reachable rate **60%** (Visable builds double opt-in from zero, subscription quality higher than Alibaba mixed-consent measured 40%), marketing frequency **1 round/week** (daily send rate 14%, full pool in 7 days; benchmarked to Alibaba FY26 weekly 2x, EU compliance starts lower), CTR 10% × decay 0.6 = **6%** (open rate declines as frequency rises, Alibaba FY26 target basis), UV→AB 20%, AB→AB2 25% (measured baseline validation: buyer response to positive connection currently QDR wlw 18%/ep 36%, RFQ wlw 33%/ep 28%; 25% within range, PRD_Reminder basis). Daily-average derivation (**target: daily AB2 ≥ 8**): subscriptions 35,000 → reachable 21,000 (×60%) → daily sends 2,940 (×14%) → daily UV 176 (×6%) → daily AB 35 (×20%) → **daily AB2 8.8 (×25%)**. **Breakeven subscription baseline: 8 ÷ 25% ÷ 20% ÷ 6% ÷ 14% ÷ 60% ≈ 31,700**——target 35,000 = 110% buffer over breakeven (still ≥ 8 at CTR decayed to 5.5%; exactly at target if 32,000). Why 35,000 instead of the original 67,000: the old 67,000 implied a slow "25-day round" frequency basis; after raising frequency to 1 round/week, the breakeven subscription demand halves naturally (daily new 316 → 165; DAU 45K conversion 0.70% → 0.37%); if frequency A/B switches to 2 rounds/week (28%/day), breakeven further drops to ~16,000.
...
### 9.2 Alibaba International Measured Data（Alibaba 国际站实测，可行性佐证明细）

| Metric 指标 | Alibaba measured 国际站实测 | Note 说明 |
|------|-----------|------|
